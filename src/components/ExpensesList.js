...
class ExpensesList extends React.Component{
    ...
    handleChange = (ev) => {
        this.props.updateExpense(ev.currentTarget.id, ev.currentTarget.value, ev.currentTarget.name);
    }
    ...
    getItem = (key, category, name, price) => {
        ...
        return (
            <li key={key}>
                <select type="text" name="category" id={key} onChange={this.handleChange} value={category}> 
                    <option defaultValue> -- select an option -- </option>
                    {options}
                </select>
                <input type="text" name="name" id={key} onChange={this.handleChange} value={name}></input>
                <input type="text" name="price" id={key} onChange={this.handleChange} value={price}></input>
    ...
    render(){
        ...
        const listItems = Object.keys(expenses).map(key => {
            ...
            if(type && moment(expenses[key].date).isSame(this.state.day, type)){
                ...
                return this.getItem(key, expenses[key].category, expenses[key].name, expenses[key].price);
            }
        });
            ...
            <div>
                {this.state.showByDay ? (
                    <div>
                        ...
                        <button onClick={() => this.changeDate(1)}>&#8594;</button>
                        <button onClick={() => this.setShowByDay(false)}>Mostrar por Semana</button>
                    </div>
                ) : (
                    <div>
                        <button onClick={() => this.changeDate(-1)}>&#8592;</button>
                        ...
                        <button onClick={() => this.changeDate(1)}>&#8594;</button>
                        <button onClick={() => this.setShowByDay(true)}>Mostrar por Día</button>
                    </div>
                )}
            </div>
        );
